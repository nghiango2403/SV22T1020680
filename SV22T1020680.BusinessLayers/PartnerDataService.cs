...
public static class PartnerDataService
{
    ...
    {
        if (data == null)
            throw new ArgumentNullException(nameof(data), "Dữ liệu nhà cung cấp không được để trống.");
        if (string.IsNullOrWhiteSpace(data.SupplierName))
            throw new Exception("Tên nhà cung cấp không được để trống.");

        if (string.IsNullOrWhiteSpace(data.ContactName))
            throw new Exception("Tên giao dịch không được để trống.");
        ...
        {
            if (!data.Phone.All(char.IsDigit))
                throw new Exception("Số điện thoại chỉ được chứa các ký tự số.");
        }

        if (string.IsNullOrWhiteSpace(data.Province))
            throw new Exception("Vui lòng chọn tỉnh thành cho nhà cung cấp.");
    ...
    {
        if (data == null)
            throw new ArgumentNullException(nameof(data), "Dữ liệu khách hàng không được để trống.");
        if (string.IsNullOrWhiteSpace(data.CustomerName))
            throw new Exception("Tên khách hàng không được để trống.");
        if (string.IsNullOrWhiteSpace(data.ContactName))
            throw new Exception("Tên giao dịch không được để trống.");
        if (string.IsNullOrWhiteSpace(data.Province))
            throw new Exception("Vui lòng chọn tỉnh thành cho khách hàng.");
        if (string.IsNullOrWhiteSpace(data.Email))
            throw new Exception("Vui lòng nhập địa chỉ Email cho khách hàng.");
        var model = await customerDB.ValidateEmailAsync(data.Email);
        if(!model)
        {
    ...
    public static async Task<bool> UpdateCustomerAsync(Customer data)
    {
        ...
        if (string.IsNullOrWhiteSpace(data.Email))
            throw new Exception("Vui lòng nhập địa chỉ Email cho khách hàng.");
        var model = await customerDB.ValidateEmailAsync(data.Email, data.CustomerID);
        if (!model)
        {
            throw new Exception("Địa chỉ Email đã tồn tại trong hệ thống.");
        }
    ...
    /// Khác 0 nếu kiểm tra email của khách hàng có mã là customerID
    /// </param>
    ...
    public static async Task<bool> RegisterCustomer(Customer customer, string password)
    {
        if (customer == null)
            throw new ArgumentNullException(nameof(customer), "Dữ liệu khách hàng không được để trống.");
        if (string.IsNullOrWhiteSpace(customer.CustomerName))
            throw new Exception("Tên của quý khách không được để trống.");
        ...
        if (string.IsNullOrWhiteSpace(customer.Email))
            throw new Exception("Vui lòng nhập địa chỉ Email cho khách hàng.");
        var model = customerDB.ValidateEmailAsync(customer.Email).Result;
        if (!model)
        {
            throw new Exception("Địa chỉ Email đã tồn tại trong hệ thống.");
        }
        return await customerDB.Register(customer, password);
    }
    /// <summary>
    ...
    {
        if (data == null)
            throw new ArgumentNullException(nameof(data), "Dữ liệu người giao hàng không được để trống.");
        if (string.IsNullOrWhiteSpace(data.ShipperName))
            throw new Exception("Tên người giao hàng không được để trống.");
        if (string.IsNullOrWhiteSpace(data.Phone))
        {
            throw new Exception("Số điện thoại của người giao hàng là thông tin bắt buộc.");
        }
        if (!System.Text.RegularExpressions.Regex.IsMatch(data.Phone, @"^0\d{9,10}$"))
        {
            throw new Exception("Số điện thoại không hợp lệ (phải bắt đầu bằng số 0 và có từ 9-10 chữ số).");
        }
