...
namespace SV22T1020680.BusinessLayers
{
    ...
    public static class CatalogDataService
    {
        ...
        {
            if (data == null)
                throw new ArgumentNullException(nameof(data), "Dữ liệu loại hàng không được để trống.");
        ...
        {
            if (data == null)
                throw new ArgumentNullException(nameof(data), "Dữ liệu mặt hàng không được để trống.");

            if (string.IsNullOrWhiteSpace(data.ProductName))
                throw new Exception("Tên mặt hàng là thông tin bắt buộc.");
            if (string.IsNullOrWhiteSpace(data.Unit))
                throw new Exception("Vui lòng nhập đơn vị tính cho mặt hàng.");

            if (data.Price < 0)
                throw new Exception("Giá mặt hàng không được nhỏ hơn 0.");

            if (data.CategoryID <= 0)
                throw new Exception("Vui lòng chọn loại hàng hợp lệ.");

            if (data.SupplierID <= 0)
                throw new Exception("Vui lòng chọn nhà cung cấp hợp lệ.");
        ...
        {
            if (data == null)
                throw new ArgumentNullException(nameof(data), "Dữ liệu thuộc tính không được để trống.");
