...
function paginationSearch(event, form, page) {
    ...
    // Xử lý các input sử dụng AutoNumeric (money-input) để lấy giá trị số thuần túy
    form.querySelectorAll('.money-input').forEach(input => {
        if (typeof AutoNumeric !== 'undefined' && AutoNumeric.getAutoNumericElement(input)) {
            formData.set(input.name, AutoNumeric.getAutoNumericElement(input).getNumericString());
        }
    });
